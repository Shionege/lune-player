...
for artist, title, target_dur in queries:
    ...
    # Search SoundCloud for title + artist
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5, context=ctx) as res:
            ...
            # Sort/filter collection to find the most ORIGINAL recording:
            # Criteria: Title does NOT contain 'cover', 'remix', 'slowed', 'reverbed', 'karaoke', 'instrumental'
            # And duration is close to target_dur (+/- 15 seconds)
            banned_keywords = ['cover', 'remix', 'slowed', 'reverb', 'karaoke', 'instrumental', 'acoustic', '8d', 'nightcore']
            ...
            if original_candidates:
                ...
                print(f"  ✅ [100% ORIGINAL STUDIO TRACK FOUND] Title: '{best_track.get('title')}' by {best_track.get('user', {}).get('username')} ({b_dur}s)")
            else:
                print("  No candidate passed strict original filter, fallback to top result...")
