...
for q in queries:
    ...
    # Step 1: Fetch Official Master Metadata from Deezer
    ...
    if official_title and official_artist:
        # Step 2: Search SoundCloud using exact Deezer Artist + Title
        ...
        try:
            ...
            with urllib.request.urlopen(sreq, timeout=4, context=ctx) as sres:
                ...
                # Match candidates by duration proximity (+/- 8 seconds) and non-banned keywords
                ...
                if matched:
                    ...
                    b_user = best.get('user', {}).get('username')
                    print(f"  ✅ [100% GENUINE STUDIO MASTER MATCHED!] Title: '{best.get('title')}' by {b_user} ({b_dur}s vs target {official_dur}s)")
                else:
