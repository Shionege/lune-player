...
for q in queries:
    ...
    for idx, s in enumerate(sources[:6], 1):
        clean_title = str(s['title']).encode('ascii', 'ignore').decode('ascii')
        clean_uploader = str(s['uploader']).encode('ascii', 'ignore').decode('ascii')
        print(f"  [{idx}] [{s['provider']}] Title: '{clean_title}' | Uploader: '{clean_uploader}' | Dur: {s['duration']}")
